...
import zhewuzhou.me.week7.copy.SeamCarver;
...
public class SeamCarverTest {
    ...
    public void should_throw_exception_given_invalid_row_negative() {
        SeamCarver carver = createFromFile(SEAM_3_X_4_PNG);
    ...
    public void should_calculate_energy_for_valid_coordinate() {
        ...
        assertThat(carver.energy(1, 2), is(Math.sqrt(52024)));
        assertThat(carver.energy(1, 2), is(Math.sqrt(52024)));
        assertThat(carver.energy(1, 1), is(Math.sqrt(52225)));
        assertThat(carver.energy(0, 0), is(1000D));
        assertThat(carver.energy(2, 3), is(1000D));
    }

    @Test
    public void should_find_vertical_seam() {
        SeamCarver carver = createFromFile(SEAM_6_X_5_PNG);

        assertThat(carver.energy(3, 0), is(1000D));
        assertThat(carver.energy(4, 1), is(closeTo(107.89, 0.01)));
        assertThat(carver.energy(3, 1), is(closeTo(234.09, 0.01)));
        assertThat(carver.energy(3, 2), is(closeTo(133.07, 0.01)));
        assertThat(carver.energy(2, 3), is(closeTo(174.01, 0.01)));
        assertThat(carver.energy(2, 4), is(1000D));
        int[] verticalSeam = {3, 4, 3, 2, 1};
        assertThat(carver.findVerticalSeam(), is(verticalSeam));
    }

    @Test
    public void should_run_with_more_cases() {
        SeamCarver carver = createFromFile(this.imagePath);

        assertThat(carver.findVerticalSeam(), is(this.seam));
    }
    ...
    public void should_find_seam_horizontal() {
        ...
        assertThat(carver.findHorizontalSeam(), is(horizontalSeam));
    }

    @Test
    public void should_find_seam_horizontal_stripes() {
        SeamCarver carver = createFromFile("seam/stripes.png");
    ...
    public void should_find_seam_vertical_image_large() {
        SeamCarver carver = createFromFile("seam/chameleon.png");

        int[] seam = {0, 1, 1, 1, 1, 1, 1, 1, 0};
        assertThat(carver.findVerticalSeam(), is(seam));
    }
